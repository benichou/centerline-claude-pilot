...
**AWS + Anthropic is your daily stack**, which makes the AWS production story on slide 25 credible, not
borrowed.
...
- *"Who do you start with?"* → Sarah, her trust scar is answered by grounding, her fear by the RM-private
  design, and she trains the team (covered on slide 24).
...
## 11 · Skills, a capability map

**What it's doing.** The skill-library companion to the architecture slide: the same library plotted on two
axes so the panel sees structure, not a list. **X = workflow stage** (understand the data → produce output);
**Y = who it serves** (risk / CCO on top, relationship / RM below). A gold **foundation-guards** band wraps
the top (every output) and a purple **eval & governance** band the bottom (measures everything), echoing the
architecture's "guards wrap, eval measures."

**Have cold.** 19 skills, eight clusters. Track A (covenant / deterioration / engagement / watchlist) sits in
risk + sense/detect; Track B's creative core (retention, cross-source reconciliation, commitment-fulfillment)
sits in relationship; drafting + CRM sit in draft/act. The empty top-right cell is deliberate.

**Likely questions → answer.**
- *"Why is the top-right empty?"* → Honest by design: risk skills *inform*, they don't draft or act, the RM
  acts. The map shows coverage and its boundaries.
- *"Watchlist vs retention radar?"* → Inverses: the watchlist surfaces distress and is blind to a healthy
  borrower who's leaving; the retention radar catches exactly that.
- *"Aren't 19 skills a lot?"* → They're small and composable; all 19 ship in the one plugin, ~15 built to depth
  with evals, the rest narrated as designed. The map shows they cluster into a handful of capabilities, reused
  across both tracks.

**Gotcha.** This is a *map*, not a build-status chart. If asked "are all 19 live?", point to the honest-eval
slide: ~15 to depth on real data; doc-intel on synthetic; a couple narrated as designed.

## 12 · Fix Tom's flagged workflow (Track A intro)
...
## 13 · A1, "Who needs attention this week?" (live)
...
## 14 · A2, "Is Meridian compliant, cushion *and* trend?" (live)
...
## 15 · A3 ★, "Who have I gone quiet on?" (live; Deliverable-A find)
...
## 17 · B1, "Prep me for my Crestwood meeting" (live; retention radar)
...
## 18 · B2 ★, "What's in the package, and do the ratios reconcile?" (live; doc-intel)
...
## 19 · B3 ★, "Reconcile the emails vs the CRM log" (live; close-the-loop)
...
## 20 · B4, "Build my Summit annual relationship-review memo" (live; the skeptic's wedge)
...
## 21 · How the CCO can trust it, four eval layers + CI gate
...
observability = OTel → Datadog / AgentCore Observability (slide 25).

## 22 · Governance, live, "How is it doing, and what should we change?"
...
## 23 · What it does well, and the hard 20%
...
## 24 · Adoption, a portfolio, not one tool
...
## 25 · From pilot to production (+ AWS path)
...
## 26 · Close, Built. Trustworthy. Adopted.
...
## 27 · Appendix · what each skill achieves

**What it's doing.** Reference, not part of the ~45-min flow. One line per skill (definition + what it
achieves), grouped by the eight clusters from the map (slide 11). Pull it up if a panelist asks "walk me
through the skills."

**Have cold.** Same eight clusters as slide 11. Full text also lives in `skills-and-mcp-glossary.md`.

**Likely questions → answer.**
- *"Which are the creative ones?"* → verifying-commitment-fulfillment (close-the-loop),
  detecting-cross-source-discrepancies (reconciliation), flagging-renewal-and-retention (retention), plus
  measuring-engagement-coverage (the Deliverable-A find).
- *"detecting-cross-source-discrepancies vs verifying-commitment-fulfillment?"* → Record integrity (is the
  file wrong / incomplete?) vs follow-through (was the plan executed?).

**Gotcha.** Don't read it line by line live; jump to the cluster the question is about.

## 28 · Appendix · the centerline MCP server + its tools

**What it's doing.** Reference. Defines the MCP server (the cross-surface compliance chokepoint) and its 19
tools in five buckets. Pull up for "what does the MCP layer actually do?"

**Have cold.** Server = local Python, runs host-side via `uv`, loaded by Code (`.mcp.json`) and bridged into
Cowork (`claude_desktop_config.json`, type: sdk). Buckets: Retrieval (6, §2.1/§5 enforced) · Track-A analysis
(5, deterministic) · Document intelligence (4) · Compliance & output (`screen_and_finalize`, `render_pdf`) ·
Eval & governance (`run_evals`, `get_latest_report`). Full text in `skills-and-mcp-glossary.md`.

**Likely questions → answer.**
- *"Why is the server the chokepoint and not hooks?"* → Hooks are Code-only; the server runs on both surfaces
  (same as slide 10).
- *"Which tools call the model vs are deterministic?"* → Only `classify_document` / `extract_document_fields`
  (doc-intel) call the Anthropic API (temp 0, forced schema); everything else, all the math and the §4.2
  screen, is deterministic Python, so the certified-vs-recomputed catch is *not* a model opinion.

**Gotcha.** `cross_validate_covenant` and `review_package` are deterministic (no API); don't let anyone read
the $2M catch as the model's guess.
